...
async function getDataSites() { // Faz requisição dos "Ramais" ao "DataBase"
	// try {
		editLegends("Sites")
		createLoad(); // Cria elemento animado de "loading"
		const api = await fetch(`${URLBack}/getSites`)
		const data = await api.json()
		// if (data.length > 0) {
			console.log(data)
...
function newFormCreatePlans() { // Cria formulário para inclusão de um novo "Plano"
	...
	<form action="${URLBack}/createPlan" method="post">
...
function newFormCreateRamal() { // Cria formulário para inclusão de um novo "Ramal"
	formCreate.innerHTML = `
        ...
        <form action="${URLBack}/createRamal" method="post">
            <input type="text" name="setor" placeholder="Contato">
            <input type="text" name="ramal" placeholder="Telefone">
            <button type="submit">Criar</button>
        </form>`
}
...
function newFormCreateDocs() { // Cria formulário para inclusão de um novo "Documento"
	...
	<form action="${URLBack}/createDoc" method="post" enctype = "multipart/form-data">
		<input type="text" name="name" placeholder="Nome" required>
...
function newFormCreateSites() { // Cria formulário para inclusão de um novo "Site"
	formCreate.innerHTML = `
	<label>Criar Site</label>
	<form action="${URLBack}/createSite" method="post">
		<input type="text" name="name" placeholder="Nome" required>
...
function newCardPlanHtml(e) { // Cria o HTML do "plano" com os dados do DataBase
	const html = `
        ...
        <form action="${URLBack}/updatePlan" id="formEdit" method="post">
            <input type="text" name="_id" disabled value="${e._id}" style="display: none ">
...
function NewCardDocsHtml(e) { // Cria o HTML do "Documento" com os dados do DataBase
    ...
    <p>${e.update.slice(0, 10).split("-").reverse().join("/")}</p>
    <button class="btnExtend"><i class="fa-solid fa-caret-down"></i></button>
    <form action="${URLBack}/updateDoc" id="formEdit" method="post" enctype = "multipart/form-data">
        <input type="text" name="_id" disabled value="${e._id}" style="display: none">
...
function NewCardRamalHtml(e) { // Cria o HTML do "ramal" com os dados do DataBase
    ...
    <p>${e.update.slice(0, 10).split("-").reverse().join("/")}</p>
    <button class="btnExtend"><i class="fa-solid fa-caret-down"></i></button>
    <form action="${URLBack}/updateRamal" id="formEdit" method="post">
        <input type="text" name="_id" disabled value="${e._id}" style="display: none">
...
function NewCardSitesHtml(e) { // Cria o HTML do "ramal" com os dados do DataBase
    ...
    <p>${e.update.slice(0, 10).split("-").reverse().join("/")}</p>
    <button class="btnExtend"><i class="fa-solid fa-caret-down"></i></button>
    <form action="${URLBack}/updateSite" id="formEdit" method="post">
        <input type="text" name="_id" disabled value="${e._id}" style="display: none">
        <label for="">Nome:</label>
        <input type="text" name="name" id="nome" disabled value="${e.name}">
...
function createNull() { // Cria elemento animado de "loading"
	list.innerHTML = ""
	let load = document.createElement("div")
	// load.classList = "loading"
	load.innerHTML = `
